...
class Extractor(Agent_Base):
    ...
    def get_related_column(
        ...
        embeddings = get_embedding_list([question] + col_name_list + comment_list)
    ...
    def get_related_value(
        ...
        for column in schema:
            ...
            if col_type.lower() == 'text':
                ...
                embeddings = get_embedding_list([question] + value_list)
                question_embedding = embeddings[0]
    ...
    def chat(
        ...
        if message["message_to"] != EXTRACTOR:
            ...
            schema, col_name_list, comment_list = self.get_schema(db_conn=db_conn)
            hint_set = self.get_rela_hint_keys(
                question=message['question'], vectordb=vectordb
            )
            col_set, strong_rela_col_set = self.get_related_column(
                question=message['question'], col_name_list=col_name_list, comment_list=comment_list, schema=schema
            )
            value_set, strong_rela_value_set = self.get_related_value(
                question=message['question'], schema=schema, db_conn=db_conn
            )
